...
export const Navbar = (): JSX.Element => {
  ...
  return (
    <div className="font-nav">
      {/* ================= MOBILE HEADER ================= */}
      <header className="lg:hidden fixed top-0 left-0 w-full z-[100]">
        <nav className="flex items-center justify-between px-6 py-4 bg-white">
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center relative cursor-pointer z-[600]"
            aria-label="Toggle menu"
          >
            <span
              className={`
                absolute h-0.5 transition-all duration-500
                ${menuOpen ? "w-5 rotate-45 bg-black" : "w-5 rotate-0 bg-black"}
              `}
              style={{
                top: menuOpen ? "50%" : "35%",
      ...
      {menuOpen && (
        <div className="fullscreen-menu fixed inset-0 z-[300] bg-black text-white pointer-events-auto overflow-y-auto">
          <div className="h-full flex flex-col justify-between px-6 lg:px-20 py-20 pt-24 lg:pt-32 max-w-2xl">
            <nav className="flex flex-col items-start gap-6 lg:gap-10 text-4xl lg:text-7xl">
              {navLinks.map((link) => (
                <Link
                  key={link.page}
                  to={link.page}
            ...
            <div className="absolute bottom-6 lg:bottom-20 right-6 lg:right-20 flex gap-4 lg:gap-10 text-sm lg:text-lg">
