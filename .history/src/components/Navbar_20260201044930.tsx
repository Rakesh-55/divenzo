...
import { useMagneticHamburger } from "../hooks/useMagneticHamburger";
...
export const Navbar = (): JSX.Element => {
  ...
  const { buttonRef: mobileHamburgerRef } = useMagneticHamburger();
  const { buttonRef: desktopHamburgerRef } = useMagneticHamburger();
      ...
      <header className="lg:hidden fixed top-0 left-0 w-full z-[500]">
        ...
        >
          ...
          >
            <span
              className={`
                absolute h-0.5 ${menuOpen ? "w-5 rotate-45" : "w-5 rotate-0"} ${menuOpen || onDarkSection ? "bg-white" : "bg-black"}
              `}
              style={{
                top: menuOpen ? "50%" : "35%",
