...
export const Navbar = (): JSX.Element => {
  ...
  return (
    <div className="font-nav">
        ...
        <div className="flex items-center gap-12">
          ...
          <Link
            ...
            className={`
              ...
              ${menuOpen ? "text-white" : onDarkSection ? "text-white" : "text-black"}
              group
              transition-all duration-500
              ${scrolled && !menuOpen ? "-translate-x-4" : ""}
            `}
          >
            Discuss with us
            <span
          ...
          {scrolled && (
            ...
            >
              <div className="relative w-5 h-5">
                <span
                  className={`
                    absolute left-1/2 top-1/2 h-[2px] w-5 transition-all duration-500 origin-center
                    ${menuOpen ? "rotate-45 -translate-x-1/2 -translate-y-1/2 bg-white" : onDarkSection ? "-translate-x-1/2 top-1 bg-white" : "-translate-x-1/2 top-1 bg-black"}
                  `}
                />
                ...
                <span
                  className={`
                    absolute left-1/2 top-1/2 h-[2px] w-5 transition-all duration-500 origin-center
                    ${menuOpen ? "-rotate-45 -translate-x-1/2 -translate-y-1/2 bg-white" : onDarkSection ? "-translate-x-1/2 bottom-1 bg-white" : "-translate-x-1/2 bottom-1 bg-black"}
                  `}
                />
              </div>
            </button>
      ...
      {/* FULLSCREEN MENU */}
      {menuOpen && (
        <div className="fullscreen-menu fixed inset-0 z-[300] bg-black text-white pointer-events-auto">
          <div className="h-full flex flex-col justify-between px-20 py-20">
            <nav className="flex flex-col items-start gap-10 text-6xl">
              {navLinks.map(link => (
                <Link
                  key={link.page}
                  to={link.page}
