...
export const ScrollDrivenStickySection: React.FC<ScrollDrivenStickySectionProps> = ({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const headingStackHeight = 60; // Space each heading takes when stacked
      const myStickyPosition = headerHeight + (index * headingStackHeight);

      // Check if this section is at its sticky position
      const isAtStickyPosition = rect.top <= myStickyPosition && rect.bottom > myStickyPosition;
      setIsActive(isAtStickyPosition);

      // Calculate collapse progress for content
      if (rect.top > myStickyPosition) {
        // Not yet reached sticky position
        setCollapseProgress(0);
      } else if (rect.top <= myStickyPosition) {
        // At or past sticky position - calculate how far past
        const distancePast = myStickyPosition - rect.top;
        const collapseDistance = 300; // Distance in pixels to fully collapse
  ...
  const headingSizeMultiplier = 1 - (Math.min(index, 3) * 0.1); // Reduce by 10% for each stacked item, max 30%
  ...
  return (
    ...
    >
      {/* Sticky Heading - Stacks below previous ones */}
      <div
        className="sticky z-40 bg-black/98 backdrop-blur-md border-b border-white/5 px-4 sm:px-10 md:px-20 py-4"
        style={{
          top: `${stickyTop}px`,
        }}
      >
        <h2
          ...
          style={{
            fontSize: `clamp(18px, ${40 * headingSizeMultiplier}px, 48px)`,
          }}
        >
          {number}. {title}
        </h2>
      ...
      {/* Collapsing Content - Only visible when this section is active */}
      <div
