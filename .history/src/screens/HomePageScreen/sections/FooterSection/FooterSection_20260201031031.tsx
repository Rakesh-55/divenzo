...
export const FooterSection = (): JSX.Element => {
  ...
  useEffect(() => {
    if (!companyRef.current || !contentRef.current) return;

    const letters = companyRef.current.querySelectorAll(".company-letter");

    // Wave animation for company name - scroll down: left to right, scroll up: reverses
    gsap.fromTo(
      letters,
      {
        y: 140,
          ...
          {socialLinks.map((link, index) => (
            ...
            >
              <span className="relative block">
                ...
                <span className="absolute inset-0 translate-y-full transition-transform duration-500 ease-out group-hover:translate-y-0">
                  {link.name}
                </span>
              </span>
            </Button>
              ...
              {navigationLinks.map((link, index) => (
                <Link key={index} to={link.page}>
                  <Button
                    variant="link"
                    className="
                      h-auto p-0 w-fit justify-start overflow-hidden
                      hover:bg-transparent focus:bg-transparent active:bg-transparent
                      [font-family:'Poppins',Helvetica] font-normal text-white
                      text-[16px] sm:text-[18px] lg:text-2xl
                      group
