...
export const FooterSection = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Only trigger when scrolling DOWN and animation hasn't run
      if (isScrollingDown && !hasAnimatedRef.current && !animationTriggered) {
        const logo = companyRef.current;
        if (!logo) return;

        const rect = logo.getBoundingClientRect();
        const isNearBottom = rect.top < window.innerHeight * 0.8 && rect.top > 0;

        if (isNearBottom) {
          animationTriggered = true;
          hasAnimatedRef.current = true;

          const letters = logo.querySelectorAll(".company-letter");

          // Letter-by-letter slide-up reveal (scroll-down triggered)
          gsap.fromTo(
            letters,
            {
              y: 100,
